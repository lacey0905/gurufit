<?php include "../inc/header.php" ?>
<section class="sub_layout">
	<div class="sub_locate">
		<div class="custom_inner">
			<dl class="locate_list">
				<dt><a href="#">홈</a></dt>
				<dd><a href="#">스토어</a></dd>
				<dd><a href="#">남자</a></dd>
			</dl>
			<p class="locate_msg">나이키 플라이니트 최대~50% OFF</p>
		</div>
	</div>
	
	<!-- 사진등록 -->
	<div class="f_modal_layer open m_size_sevenfive photo_register_md">
		<div class="f_modal_wrap">
			<div class="f_modal_area">
        		<div class="f_modal_head">
        			<h2>사진 등록<span>사진을 등록할 상품을 선택해주세요.</span></h2>
        			<a href="#" class="btn_close">Close</a>       			
        		</div>
        		<div class="f_modal_cont">
        			<div class="f_modal_service">
						<table class="prd_list_a">
							<caption class="blind">장바구니</caption>
							<colgroup>
								<col class="col_thum">
								<col class="col_cont">
							</colgroup>
							<tbody>
								<tr>
									<td class="thum">
										<span class="prd_thum">
				    						<a href="#">
				    							<img src="../img/store_sample_230x160.png" alt="샘플상품">
			    							</a>
			   							</span>
									</td>
									<td class="cont">
										<span class="brand"><img src="../img/nike_x34.png" alt="나이키" ></span>
										<strong class="name">Nike Metcon DSX Flyknit</strong>
										<p>구입한 날짜 2017년 7월 14일</p>
    									<div class="rating">
    			    						<strong class="star_rating star_mid">
    			    							<i class="star_on"></i>
    			    							<i class="star_on"></i>
    			    							<i class="star_on"></i>
    			    							<i class="star_on"></i>
    			    							<i class="star_off"></i>
    			   							</strong>
    			    						<span class="pr_review_txt">마음에 들어요!</span>    			   							
    			    					</div>
									</td>
								  </tr>

							 </tbody>
						</table>
                        <div class="photo_register_text">
                        	<img src="../img/photo-1.png" alt="샘플상품">
                        	<p>
                                                    신발과 함께 스타일을 보여줄 수 있는 사진을 등록 하세요.<br>
                            <span>Tip : 사진이 등록되면 경험치 100xp가 올라 갑니다.</span>                        		
                        	</p>
                        	<a href="#" class="btn_ btn_xl_l4"><i class="fa fa-facebook-official" aria-hidden="true"></i>페이스북 사진 가져오기</a>
                        	<a href="#" class="btn_ btn_xl_l4"><i class="fa fa-instagram" aria-hidden="true"></i>인스타그램 사진 가져오기</a>
                        	<a href="#" class="btn_ btn_xl_l4"><i class="material-icons">&#xE2C6;</i>파일 선택</a>
                        </div>    						  							
        			</div>
        		</div>        		
    		</div>
		</div>
		<div class="f_modal_blind"></div>
	</div>
	
	
</section>
<?php include "../inc/footer.php" ?>